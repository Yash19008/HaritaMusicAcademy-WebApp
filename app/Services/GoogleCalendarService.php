...
class GoogleCalendarService
{
    ...
    private function resolveCalendarCredentials(): array
    {
        ...
        return [$token, $calendarId, $timezone, $impersonateEmail];
    }

    private function buildAttendees(ClassBooking|DemoBooking $booking): array
    {
        ...
        if ($booking instanceof DemoBooking) {
             if ($booking->email) {
                 $attendees[] = ['email' => $booking->email, 'displayName' => $booking->student_name ?? 'Student'];
             }
             return array_values(array_filter($attendees));
        }
    ...
    private function getClassName(ClassBooking|DemoBooking $booking): string
    {
        if ($booking instanceof DemoBooking) {
            return ($booking->instrument ?? 'Music') . ' Demo Class - ' . ($booking->student_name ?? 'Student');
        }

        if ($booking->student_group_id) {
            return $booking->studentGroup->name . ' Group Class';
        } else {
            $studentName  = $booking->student->user->name ?? $booking->student->name ?? 'Student';
            return ($booking->instrument ?? 'Music') . ' Class - ' . $studentName;
        }
    }
    ...
    public function createMeetEvent(ClassBooking|DemoBooking $booking): GoogleCalendarEventResult
    {
        ...
        try {
            [$token, $calendarId, $timezone] = $this->resolveCalendarCredentials();
            if (! $token) {
                return new GoogleCalendarEventResult('failed', message: 'Unable to create Google access token.');
            }

            $attendees = $this->buildAttendees($booking);
            $className = $this->getClassName($booking);
            ...
            $response = Http::timeout(10)->retry(2, 500)->withToken($token)
                ->withQueryParameters([
                    ...
                    'sendUpdates'           => config('services.google.send_updates'),
                ])
                ->post(
    ...
    public function createRecurringEvent(ClassBooking $masterBooking, \Illuminate\Database\Eloquent\Collection $bookings): GoogleCalendarEventResult
    {
        ...
        try {
            ...
            $attendees = $this->buildAttendees($masterBooking);
            ...
            $payload = [
                ...
                'conferenceData' => [
                    'createRequest' => [
                        'requestId' => 'harita-rec-' . $masterBooking->id . '-' . Str::uuid(),
                    ],
                ],
            ];
            ...
            if (! $response->successful()) {
                Log::error('Google Calendar createRecurringEvent failed', ['response' => $response->json()]);
                return new GoogleCalendarEventResult('failed', payload: $response->json(), message: $response->body());
            }

            $event = $response->json();
            
            // Bug #1 fix: Do not updateQuietly here. Let SyncGoogleCalendar do it.
            ...
            Log::error('Google Calendar createRecurringEvent exception: ' . $throwable->getMessage());
            return new GoogleCalendarEventResult('failed', message: $throwable->getMessage());
        }
    }
    ...
    public function deleteEventInstance(ClassBooking $booking): GoogleCalendarEventResult
    {
        ...
        try {
            ...
            $originalStartTimeStr = $booking->google_original_start;
            if (!$originalStartTimeStr) {
                $originalStartTimeStr = $booking->starts_at->copy()->setTimezone('UTC')->format('Ymd\THis\Z');
            }
    ...
    public function updateEventInstance(ClassBooking $booking, \Carbon\Carbon $originalStartsAt): GoogleCalendarEventResult
    {
        ...
        try {
            ...
            if (! $response->successful()) {
                Log::error('Google Calendar updateEventInstance failed', ['response' => $response->json()]);
                return new GoogleCalendarEventResult('failed', payload: $response->json(), message: $response->body());
            }
            ...
            Log::error('Google Calendar updateEventInstance exception: ' . $throwable->getMessage());
            return new GoogleCalendarEventResult('failed', message: $throwable->getMessage());
        }
    }
    ...
    private function fetchTokenFromGoogle(?string $impersonate)
    {
    ...
    private function accessToken(?string $impersonate = null): ?string
    {
        ...
        return Cache::remember('google_access_token_' . md5($impersonate ?? 'default'), 3500, function() use ($impersonate) {
            $response = $this->fetchTokenFromGoogle($impersonate);
            ...
            if (! $response->successful()) {
                ...
                if ($impersonate && str_contains($response->body(), 'invalid_grant')) {
                    $fallbackResponse = $this->fetchTokenFromGoogle(null);
                    if ($fallbackResponse && $fallbackResponse->successful()) {
                        return $fallbackResponse->json('access_token');
                    }
                }
                return null;
            }
