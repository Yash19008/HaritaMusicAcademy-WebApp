...
<head>
    ...
    <title>Recurring Class Booking Summary</title>
...
<body>
    <div class="email-container">
        <div class="header">
            ...
            <p>Your {{ $firstBooking->instrument ?? 'Music' }} Sessions are Scheduled</p>
        </div>

        <div class="content">
            <div class="greeting">
                Dear
                <strong>{{ $isTeacher ? $firstBooking->teacher->user->name ?? 'Teacher' : $targetStudent->name ?? $firstBooking->student->name ?? 'Student' }}</strong>,
            </div>

            <p class="info-text">
                Your recurring music classes have been successfully scheduled. A total of <strong>{{ $totalClasses }}
                    classes</strong> have been booked.
            </p>
            ...
            <div class="details-box">
                ...
                <div class="detail-item">
                    ...
                    <span class="detail-value">{{ $firstBooking->instrument ?? 'N/A' }}</span>
                </div>
                <div class="detail-item">
                    <span class="detail-label">{{ $isTeacher ? 'Student:' : 'Instructor:' }}</span>
                    <span class="detail-value">{{ $isTeacher ? ($firstBooking->student_group_id ? $firstBooking->studentGroup->name : ($firstBooking->student->name ?? 'N/A')) : ($firstBooking->teacher->user->name ?? 'N/A') }}</span>
                </div>
                <div class="detail-item">
                    <span class="detail-label">Total Classes:</span>
                    <span class="detail-value">{{ $totalClasses }}</span>
                </div>
            </div>

            <p class="info-text" style="margin-top: 25px;"><strong>Your Scheduled Dates:</strong></p>
