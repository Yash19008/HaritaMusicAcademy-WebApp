...
@section('content')
    ...
    <div class="profile-hero slide-up">
        ...
        <div class="profile-meta">
            ...
            <p id="profileSub">
                {{ $student ? ($student->course ? $student->course->name : $student->enrolled_format) : 'Student' }} |
                Harita Music Academy</p>
        </div>
    </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="info-list-item">
                    ...
                    <span
                        class="font-semibold">{{ $student ? ($student->course ? $student->course->name : '-') : '-' }}</span>
                </div>
                <div class="info-list-item">
                    <span class="text-muted">Assigned Mentor</span>
                    <span
                        class="font-semibold">{{ $student && $student->teacher ? $student->teacher->user->name : 'Not Assigned' }}</span>
                </div>
                <div class="info-list-item">
                ...
                <div class="info-list-item">
                    ...
                    <span
                        class="font-semibold">{{ $student && $student->joining_date ? \Carbon\Carbon::parse($student->joining_date)->format('F d, Y') : '-' }}</span>
                </div>
                <div class="info-list-item">
                    <span class="text-muted">Learning Level</span>
                    <span
                        class="font-semibold text-success">{{ $student && $student->enrolled_level ? $student->enrolled_level : '-' }}</span>
                </div>
            </div>
        </div>
                    ...
                    @foreach ($payments as $payment)
                        <div class="info-list-item">
                            <div>
                                ...
                                <div class="text-muted" style="font-size: 0.7rem;">Paid:
                                    {{ $payment->payment_mode ?? 'Online' }}</div>
                            </div>
                            <div class="text-right">
                                <div class="font-bold">₹{{ number_format($payment->amount) }}</div>
                                <div class="text-muted" style="font-size: 0.7rem;">
                                    {{ \Carbon\Carbon::parse($payment->transaction_date)->format('M d, Y') }}</div>
                            </div>
                        </div>
                    @endforeach
                @else
                    <div class="p-4 text-center text-muted"
                        style="background: var(--bg-body); border: 1px dashed var(--border-color); border-radius: var(--radius-md);">
