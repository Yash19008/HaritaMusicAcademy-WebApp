                ...
                <tbody>
                    @foreach ($leaves as $leave)
                        <tr>
                            ...
                            <td style="font-size:13px;" data-sort="{{ $leave->from_date }}">
                                {{ \Carbon\Carbon::parse($leave->from_date)->format('M d, Y') }} to
                            ...
                            <td>
                                ...
                                <span
                                    class="badge {{ $st === 'approved' ? 'badge-success' : ($st === 'rejected' ? 'badge-danger' : 'badge-warning') }}">
                                    {{ ucfirst($st) }}
                                </span>
                            </td>
                                    ...
                                    <div class="actions-dropdown-container" style="display:inline-block;">
                                        ...
                                        <div class="actions-dropdown-menu" style="min-width:140px;">
                                            <form method="POST" action="{{ route('admin.leaves.approve', $leave->id) }}"
                                                style="margin:0;">
                                                @csrf
                                                <button type="submit" class="actions-dropdown-item text-success"
