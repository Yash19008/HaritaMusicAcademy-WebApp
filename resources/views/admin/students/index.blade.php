            ...
            <div class="d-flex align-center gap-2">
              ...
              <div>
                ...
                @if($student->intro_video_path)
                  <br><span class="badge" style="font-size:10px;margin-top:3px;display:inline-block;background:rgba(16,185,129,0.15);color:#10b981;border:1px solid rgba(16,185,129,0.3);">🎥 Intro Video</span>
                @endif
              </div>
            </div>
          </td>
          ...
          <td>
            <div class="actions-dropdown-container">
              ...
              <div class="actions-dropdown-menu" style="min-width:175px;">
                ...
                @if($student->intro_video_path)
                <button class="actions-dropdown-item" style="color:#10b981;" onclick="openIntroVideo('{{ addslashes($student->name) }}', '{{ asset('storage/' . $student->intro_video_path) }}', '{{ $student->intro_video_uploaded_at ? \Carbon\Carbon::parse($student->intro_video_uploaded_at)->format('M d, Y') : '' }}')">
                  🎥 View Intro Video
                </button>
                @endif
                <form method="POST" action="{{ route('admin.students.resend-credentials', $student) }}" onsubmit="return confirm('Generate a new password and resend email?')" style="margin:0;">
                  @csrf
                  <button type="submit" class="actions-dropdown-item" style="width:100%;text-align:left;background:none;border:none;cursor:pointer;padding:.5rem .75rem;">📧 Resend Email</button>
                </form>
          ...
          <td>
            <div class="actions-dropdown-container">
              ...
              <div class="actions-dropdown-menu" style="min-width:130px;right:0;">
                <button class="actions-dropdown-item" onclick="openEditGroup({{ $group->id }}, '{{ addslashes($group->name) }}', '{{ $group->status ?? 'active' }}', [{{ $group->members->pluck('id')->join(',') }}], {{ $group->teacher_id ?? 'null' }})">✏️ Edit Group</button>
      ...
      <div class="modal-body">
        ...
        <div class="grid grid-2 gap-3">
          ...
          <div class="form-group">
            ...
            <input type="number" name="age" id="sfAge" class="form-control" placeholder="e.g. 25" min="1">
          </div>
        </div>
        <div class="grid grid-2 gap-3">
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="student-check-list" id="sfCourseList" style="max-height: 120px;">
              @foreach($courses as $course)
                <label>
                  <input type="checkbox" name="courses[]" value="{{ $course->id }}" class="sf-course-checkbox" style="width:15px;height:15px;accent-color:var(--primary);">
        ...
        <div class="grid grid-2 gap-3">
          ...
          <div class="form-group">
            ...
            <input type="date" name="joining_date" id="sfJoiningDate" class="form-control" onchange="calculateStudentEndDate()">
          </div>
        </div>
        ...
        <div class="grid grid-2 gap-3">
          ...
          <div class="form-group">
            ...
            <select id="sfCreditPackage" class="form-control" onchange="document.getElementById('sfCredits').value = this.options[this.selectedIndex].dataset.credits || 0; calculateStudentEndDate();">
              <option value="" data-credits="0">— Custom / No Package —</option>
              @if(isset($creditPackages))
                @foreach($creditPackages as $package)
                  <option value="{{ $package->id }}" data-credits="{{ $package->credits }}">{{ $package->name }} ({{ $package->credits }} Credits)</option>
                @endforeach
              @endif
            </select>
          </div>
        ...
        <div class="grid grid-2 gap-3">
          <div class="form-group">
            ...
            <input type="number" name="credits" id="sfCredits" class="form-control" value="0" min="0" onchange="calculateStudentEndDate()">
          </div>
          <div class="form-group">
          ...
          <div class="student-check-list" id="groupStudentsList">
            @forelse($students as $student)
              <label>
                ...
                <span>{{ $student->name }}
                  @if($student->courses->isNotEmpty()) <span style="font-size:11px;color:var(--text-muted)">({{ $student->courses->pluck('name')->join(', ') }})</span>@endif
                </span>
              </label>
            @empty
