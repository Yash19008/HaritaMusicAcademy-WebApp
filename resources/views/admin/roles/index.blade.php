        ...
        <div class="card p-3">
            ...
            <form id="userAccountForm" onsubmit="submitSaveUser(event)">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="password" id="userPassword" class="form-control" placeholder="Minimum 8 characters"
                        required>
                </div>
...
@push('scripts')
    ...
    <script>
            ...
            <td>
              <div class="actions-dropdown-container">
                ...
                <div class="actions-dropdown-menu" style="min-width: 130px; right: 0; top: 100%; z-index: 50;">
                  <button class="actions-dropdown-item" onclick="editUser(${u.raw_id})">✏️ Edit</button>
                  <button class="actions-dropdown-item text-danger" onclick="deleteUser(${u.raw_id})">🗑️ Delete</button>
                </div>
              </div>
            </td>
          `;
        ...
        function editUser(rawId) {
            const user = ALL_USERS.find(u => u.raw_id == rawId);
            if (!user) return;

            document.getElementById("formUserId").value = user.raw_id;
            ...
            for (let i = 0; i < roleSelect.options.length; i++) {
                if (roleSelect.options[i].text.toLowerCase().includes(user.role.toLowerCase()) ||
                    roleSelect.options[i].value.toLowerCase() === user.role.toLowerCase()) {
            ...
            document.getElementById("btnSaveUser").textContent = "Update Account";
            document.getElementById("btnCancelEdit").style.display = "block";

            // Scroll to form
            document.getElementById("userAccountForm").scrollIntoView({
                behavior: 'smooth',
                block: 'start'
            });
        }
        ...
        async function deleteUser(rawId) {
            if (rawId == 1) {
                alert(
                    "Access Denied: The primary Administrator account cannot be deleted to prevent locking out of the system!");
                return;
            }
            ...
            try {
                const response = await fetch(`/admin/users/${rawId}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
