...
@section('title', 'Syllabus Master - Harita Music Academy')
...
<div class="card">
    <div class="card-header d-flex align-center justify-between">
        <h4 class="font-semibold" style="font-family: var(--font-serif); font-size: 1.25rem;">Syllabus Master</h4>
    ...
    <div class="card-body p-3">
        <!-- Filter Form -->
        <form method="GET" action="{{ route('admin.syllabus.index') }}" class="mb-4" style="max-width: 300px;">
...
<!-- Upload/Edit Modal -->
<div class="modal-overlay" id="syllabusModal" style="display: none; position: fixed; top:0; left:0; right:0; bottom:0; background: rgba(0,0,0,0.5); z-index: 999; align-items:center; justify-content:center;">
    <div class="modal-content" style="background: white; border-radius: var(--radius-md); padding: 1.5rem; width: 100%; max-width: 500px; box-shadow: var(--shadow-lg);">
        <div class="d-flex justify-between align-center mb-3">
            <h3 id="modalTitle" style="font-family: var(--font-serif); margin:0;">Upload Syllabus</h3>
            <button type="button" onclick="closeModal()" style="background:none; border:none; cursor:pointer; font-size:1.2rem;">&times;</button>
        </div>
        
        <form id="syllabusForm" action="{{ route('admin.syllabus.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="_method" id="formMethod" value="POST">
            
            <div class="mb-3">
                <label class="form-label d-block mb-1">Course <span class="text-danger">*</span></label>
                <select name="course_id" id="course_id" class="form-control w-100" required>
...
<script>
    ...
    const storeUrl = "{{ route('admin.syllabus.store') }}";

    function openUploadModal() {
        document.getElementById('modalTitle').innerText = 'Upload Syllabus';
    ...
    function openEditModal(id, courseId, title, description, order, isActive) {
        document.getElementById('modalTitle').innerText = 'Edit Syllabus';
        document.getElementById('formMethod').value = 'PUT';
        form.action = `/admin/syllabus/${id}`;
